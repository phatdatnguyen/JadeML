...
{
    public partial class VisualizeClassificationEvaluationDataDialog : Form
    {
        #region Fields
        ...
        #region Constructor
        public VisualizeClassificationEvaluationDataDialog(string[] inputColumnNames, string outputColumnName, double[][] inputColumns, string[] predictiedOutputColumn, string[] expectedOutputColumn)
        {
            InitializeComponent();
            ...
            outputColumn = new string[predictiedOutputColumn.Length];
            for (int rowIndex = 0; rowIndex < predictiedOutputColumn.Length; rowIndex++)
                if (predictiedOutputColumn[rowIndex] == expectedOutputColumn[rowIndex])
                    outputColumn[rowIndex] = predictiedOutputColumn[rowIndex];
                else
                {
                    string predictedClassLabel = predictiedOutputColumn[rowIndex];
                    string expectedClassLabel = expectedOutputColumn[rowIndex];
                    outputColumn[rowIndex] = predictedClassLabel + " (expected " + expectedClassLabel + ")";
                }
